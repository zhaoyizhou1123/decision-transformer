...
from gym_dt.decision_transformer.evaluation.evaluate_episodes import evaluate_episode, evaluate_episode_rtg
# from gym_dt.decision_transformer.models.decision_transformer import DecisionTransformer
...
# from gym_dt.decision_transformer.training.seq_trainer import SequenceTrainer
from maze.scripts.create_maze_dataset import create_env_dataset
from maze.utils.setup_seed import setup_seed
from viskit.logging import logger , setup_logger as setup_logger_cql
...
def experiment(
    ...
    group_name = f'{exp_prefix}-{env_name}s'
    ...
    if args.task == 'pickplace':
        ...
        trajectories = get_pickplace_dataset_dt(args.data_dir)
        # args.max_action = env.action_space.high[0]
        # print(args.action_dim, type(args.action_dim

        trajectories.sort(key=lambda ele: ele.returns[0], reverse = True)
        keep_num_traj = int(len(trajectories) * args.data_ratio)
        trajectories = trajectories[:keep_num_traj]
        trajectories = [traj._asdict() for traj in trajectories]
        for traj in trajectories:
            for k in traj:
                traj[k] = np.asarray(traj[k])
            traj['dones'] = np.full_like(traj['rewards'], False)
            # traj['rewards'] = traj['rewards'].reshape(traj['rewards'].shape[0],-1)
        scale = 1
    ...
    setup_logger_cql(
        variant=vars(args),
        exp_id=f"pickplace_0.1bc",
        seed=args.seed,
        base_log_dir="./pickplace_bc_log/",
    ...
    for iter in range(variant['max_iters']):
        ...
        if log_to_wandb:
            wandb.log(outputs)
        logger.record_dict(outputs)
        logger.dump_tabular(with_prefix=False, with_timestamp=False)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--algo-name", type=str, default="bc_pickplace")
    ...
    parser.add_argument('--K', type=int, default=200) # horizon
    ...
    parser.add_argument('--maze_config_file', type=str, default='../maze/config/maze2_simple_moredata.json')
    parser.add_argument('--data_file', type=str, default='../maze/dataset/maze2_smds_acc.dat')
    ...
    experiment('gym-experiment', args, variant=vars(args))
